...
class GitConnectorConfig:
    ...
    def add_connector(self, connector: GitConnector):
        ...
        self.connectors = [
            ...
            if not (c.relay_id == connector.relay_id and c.shared_folder_id == connector.shared_folder_id)
        ]
        self.connectors.append(connector)
    ...
    def validate_config(self) -> List[str]:
        ...
        for i, connector in enumerate(self.connectors):
            ...
            if not connector.branch or '/' in connector.branch.split('/')[-1]:
                errors.append(
                    f"Invalid branch name in git_connector[{i}]: {connector.branch}"
                )
            
            # Validate remote name
            if not connector.remote_name or ' ' in connector.remote_name:
                errors.append(
                    f"Invalid remote name in git_connector[{i}]: {connector.remote_name}"
                )
