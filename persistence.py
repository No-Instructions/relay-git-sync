...
import git
from s3rn import S3RNType, S3RN, S3RemoteFolder, S3RemoteDocument, S3RemoteCanvas, S3RemoteFile, ResourceInterface
...
class PersistenceManager:
    ...
    def __init__(self, data_dir: str = "."):
        ...
        self.filemeta_folders: Dict[str, Dict[str, Dict]] = {}  # keyed by relay_id then folder doc_id
        self.local_file_state: Dict[str, Dict[str, Dict]] = {}  # keyed by relay_id then folder_id then path
            ...
            for relay_id in os.listdir(state_base_dir):
                ...
                for folder_id in filemeta_folders.keys():
                    try:
                        ...
                        logger.error(f"Failed to initialize Git repo for folder {folder_id} in relay {relay_id}: {e}")
        ...
        with self.git_lock:
            ...
            try:
                ...
                if "index.lock" in str(e) or "File exists" in str(e):
                    logger.warning(f"Git operation failed due to lock file, attempting cleanup: {e}")
                    self._cleanup_git_lock_files()
    ...
    def init_git_repo(self, relay_id: str, folder_id: str) -> git.Repo:
        ...
        try:
            ...
            print(f"Using existing git repository for folder {folder_id} in relay {relay_id} at {folder_path}")
        except git.InvalidGitRepositoryError:
            # Create new repo
            self.git_repos[repo_key] = git.Repo.init(folder_path, initial_branch="main")
            print(f"Initialized new git repository for folder {folder_id} in relay {relay_id} at {folder_path} with main branch")
    ...
    def configure_git_remote(self, relay_id: str, folder_id: str, remote_url: str, remote_name: str = "origin"):
        ...
        try:
            ...
            if remote_name in [r.name for r in git_repo.remotes]:
                ...
                print(f"Updated remote '{remote_name}' URL for folder {folder_id} in relay {relay_id}: {remote_url}")
            else:
                # Add new remote
                git_repo.create_remote(remote_name, remote_url)
                print(f"Added remote '{remote_name}' for folder {folder_id} in relay {relay_id}: {remote_url}")
            
            return True
            
        except Exception as e:
            logger.error(f"Error configuring remote for folder {folder_id} in relay {relay_id}: {e}")
            return False
    ...
    def _push_to_remote(self, repo_key: str, git_repo: git.Repo):
        ...
        try:
            ...
            origin = git_repo.remotes.origin if 'origin' in [r.name for r in git_repo.remotes] else git_repo.remotes[0]
            ...
            try:
                ...
                if current_branch.tracking_branch() is None:
                    ...
                    self._safe_git_operation(lambda: origin.push(current_branch.name, set_upstream=True))
                    print(f"Git push (set upstream) for repository {repo_key} to {origin.name}/{current_branch.name}")
                else:
                    # Regular push
                    self._safe_git_operation(lambda: origin.push())
                    print(f"Git push for repository {repo_key} to {origin.name}/{current_branch.name}")
                    
            except git.exc.GitCommandError as e:
                # Handle common push errors gracefully
                if "non-fast-forward" in str(e):
                    logger.warning(f"Push rejected for repository {repo_key}: non-fast-forward. Manual intervention may be needed.")
                elif "Permission denied" in str(e) or "Authentication failed" in str(e):
                    logger.warning(f"Push failed for repository {repo_key}: authentication error. Check SSH keys or credentials.")
                else:
    ...
    def update_local_file_state(self, document_resource: ResourceInterface, path: str, file_hash: Optional[str] = None):
    ...
    def find_local_file_by_doc_id(self, relay_id: str, folder_id: str, doc_id: str) -> Optional[str]:
    ...
    def write_file_content(self, document_resource: S3RNType, path: str, content: str, file_hash: Optional[str] = None):
    ...
    def write_binary_file_content(self, document_resource: S3RNType, path: str, content: bytes, file_hash: Optional[str] = None):
    ...
    def move_file(self, document_resource: S3RNType, from_path: str, to_path: str) -> tuple[str, str]:
    ...
    def update_resource_index_for_document(self, relay_id: str, resource_id: str, folder_id: str, path: str, metadata: Dict):
    ...
    def _create_s3rn_from_index(self, relay_id: str, resource_id: str, resource_info: Dict) -> Optional[S3RNType]:
        ...
        if resource_type == "standalone_document":
            logger.warning(f"Standalone document {resource_id} cannot be represented as proper S3RN resource")
            return None
        ...
        logger.warning(f"Cannot create S3RN resource for type '{resource_type}' (category '{s3rn_category}') with folder_id '{folder_id}' - missing folder or unsupported type")
        return None