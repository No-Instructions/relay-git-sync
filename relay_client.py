...
class RelayClient:
        ...
        if "edges" in doc.keys():
            ...
            for key in sorted(yedges.keys()):
                ...
                if isinstance(edge_data, dict):
                    ...
                    canvas_data["edges"].append(self._map_to_dict(edge_data) if isinstance(edge_data, Map) else edge_data)
    ...
    def fetch_s3_file_content(self, resource: S3RemoteFile, file_hash: str, mimetype: str = "application/octet-stream") -> Optional[bytes]:
        ...
        try:
            ...
            logger.debug(f"✅ S3 FILE DOWNLOAD RESPONSE: {download_response.status_code} ({len(download_response.content)} bytes)")
    ...
    def _get_download_url(self, resource: S3RemoteFile, file_hash: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.get(download_url_endpoint, params=params, headers=headers, timeout=10)
    ...
    def extract_relay_id(doc_id: str) -> str:
        ...
        if len(parts) < 10:  # Should be two UUIDs: 5 parts + 5 parts
            raise ValueError(f"Invalid document ID format: {doc_id}. Expected compound UUID: relay_uuid-doc_uuid")
        return '-'.join(parts[:5])
