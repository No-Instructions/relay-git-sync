...
def generate_webhook_secret():
    ...
    return f"whs_{secret_key}"

def generate_jwt_secret():
    """Generate a secure JWT signing secret"""
    secret_bytes = secrets.token_bytes(32)
    secret_key = base64.urlsafe_b64encode(secret_bytes).decode('utf-8').rstrip('=')
    return f"jwt_{secret_key}"
...
def create_jwt_token(secret, scope, expires_in_days=30, name=None):
    ...
    if secret.startswith(('whs_', 'jwt_')):
...
def webhook_token_create_command(args):
    """Handle webhook token create command"""
    try:
        webhook_secret = os.getenv('WEBHOOK_SECRET')
        if not webhook_secret:
            print("Error: WEBHOOK_SECRET environment variable is required to create tokens.")
            print("Run: python cli.py webhook keygen")
            return 1
            
        if webhook_secret.startswith('whsec_'):
            print("Error: Svix webhook secrets use HMAC signatures, not bearer tokens.")
            print("Configure your webhook provider to send Svix-style signatures.")
            return 1
        
        token = create_jwt_token(webhook_secret, 'webhook', args.expires, args.name)
        print(f"\nWebhook JWT Token (expires in {args.expires} days):")
        print("=" * 50)
        print(token)
        print("=" * 50)
        print("\nUse with webhook providers:")
        print(f"Authorization: Bearer {token}")
        return 0
    except Exception as e:
        logger.error(f"Error creating webhook token: {e}")
        return 1
...
def api_token_create_command(args):
    ...
    try:
        ...
        secret_to_use = jwt_secret or webhook_secret
        if not secret_to_use:
            print("Error: JWT_SECRET or WEBHOOK_SECRET environment variable is required to create API tokens.")
            print("Run: python cli.py api keygen")
            return 1
            
        if secret_to_use.startswith('whsec_'):
            print("Error: Svix webhook secrets cannot be used for API token generation.")
            print("Run: python cli.py api keygen")
            return 1
        
        token = create_jwt_token(secret_to_use, 'api', args.expires, args.name)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Y-Sweet Git Sync CLI - Sync collaborative documents to Git repositories",
...
Examples:
  ...
  python cli.py api token create --name "deploy-script"
    ...
    webhook_token_parser = webhook_subparsers.add_parser('token', help='Webhook token management')
    webhook_token_subparsers = webhook_token_parser.add_subparsers(dest='webhook_token_action', help='Token actions')
    
    webhook_create_parser = webhook_token_subparsers.add_parser('create', help='Create webhook token')
    webhook_create_parser.add_argument('--expires', type=int, default=30, help='Expiration in days (default: 30)')
    webhook_create_parser.add_argument('--name', help='Token name for identification')
    webhook_create_parser.set_defaults(func=webhook_token_create_command)
    ...
    api_token_subparsers = api_token_parser.add_subparsers(dest='api_token_action', help='Token actions')
    
    api_create_parser = api_token_subparsers.add_parser('create', help='Create API token')
    api_create_parser.add_argument('--expires', type=int, default=30, help='Expiration in days (default: 30)')
    ...
    if not hasattr(args, 'func'):
        ...
        if args.command == 'webhook' and not hasattr(args, 'webhook_action'):
            ...
            return 1
        elif args.command == 'webhook' and args.webhook_action == 'token' and not hasattr(args, 'webhook_token_action'):
            webhook_token_parser.print_help()
            return 1
        elif args.command == 'api' and args.api_action == 'token' and not hasattr(args, 'api_token_action'):
    ...
    if args.command == "sync":
        ...
        if not args.relay_id or len(args.relay_id.split('-')) != 5:
            print("Error: Invalid relay ID format. Expected UUID format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
            return 1
        
        # Validate folder ID format if provided (should be simple UUID)
        if args.folder_id and len(args.folder_id.split('-')) != 5:
            print("Error: Invalid folder ID format. Expected UUID format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
            return 1
